      ...
      <main className="relative mx-auto max-w-[1480px] px-4 pb-10 pt-5 sm:px-7 lg:px-10 lg:pt-7">
        <header className="mb-8 flex flex-col gap-5 border-b border-white/10 pb-5 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-3 text-xs">
            <div className={`flex items-center gap-2 rounded-full border px-3 py-2 ${snapshot.dataStatus === "fresh" ? "border-[#6CE2C2]/25 bg-[#6CE2C2]/[0.06] text-[#A8DAD0]" : "border-[#F1BE71]/25 bg-[#F1BE71]/[0.06] text-[#E7C993]"}`}><Database className="h-3.5 w-3.5" /> {snapshot.dataStatus === "fresh" ? "所有資料來源可用" : "部分資料來源不可用"}</div>
        ...
        {snapshot.dataStatus === "partial" && <div className="mb-5 flex gap-3 rounded-2xl border border-[#F1BE71]/20 bg-[#F1BE71]/[0.07] p-4"><TriangleAlert className="mt-0.5 h-4 w-4 shrink-0 text-[#F1BE71]" /><p className="text-xs leading-5 text-[#E7C993]">目前有 {staleCount} 個因子資料偏舊或暫不可用；這些因子會以中性分數處理，結論置信度已相應下調。</p></div>}
